...
Begin VB.Form frmConFallecidosxAno 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Socios Fallecidos Por Año"
   ClientHeight    =   8595
   ClientLeft      =   45
   ClientTop       =   390
   ClientWidth     =   11010
   ...
   End
End
Attribute VB_Name = "frmConFallecidosxAno"
...
Private Sub cmdExportar_Click()
   ...
   Heading(4) = "ESTADO"
   Heading(5) = "MES FALL"
   aa = Leerado3("SELECT * FROM TMP_SOCIOS WHERE USU = '" + wcodusu + "' ORDER BY MESFALL, NOMBRE ")
   If aa > 0 Then
      wTot = aa
      ...
      With objExcel
           ...
           .Cells(1, 1) = wnomcia
           .Cells(2, 1) = "RELACION DE ASOCIADOS FALLECIDOS - EJERCICIO " + txtAno.Text
           For I = 1 To 6 Step 1
      ...
      Do While Not ADO3.EOF
         ...
         objExcel.Cells(V, H + 5) = ADO3!mesfall
         wreg = wreg + 1
         V = V + 1
         ADO3.MoveNext
      Loop
...
Private Sub cmdImprimir_Click()
   ...
   Crys1.ReportFileName = xraiz + "ReportCtaCte\RepFallxAno.RPT"
...
Private Sub Form_Load()
   frmConFallecidosxAno.Left = (Screen.Width - Width) \ 2
   frmConFallecidosxAno.Top = 0
End Sub
...
Private Sub LlenaCab()
   ...
   & " (CODSOCIO, CODIGO, INS, NOMBRE, E_SOCIO, MESFALL, MONEDA, APORTE, " _
   & "  USU ) " _
   & " SELECT " _
   & "  S.CODSOCIO, S.CODIGO, S.INS, S.NOMBRE, S.E_SOCIO, S.MESFALL, " _
   & "  E.MONEDA, E.APORTE, '" + wcodusu + "' " _
   & " FROM MAESOCIO AS S INNER JOIN MAEE_SOCIO AS E " _
   & "   ON S.E_SOCIO = E.E_SOCIO " _
   & " WHERE S.MESFALL LIKE '" + wAno + "%'  AND S.E_SOCIO = 'FAL'  ")
   Db.CommitTrans

   aa = Leerado2("SELECT CODSOCIO, CODIGO, INS, NOMBRE, E_SOCIO, MESFALL, " _
                & "      MONEDA, APORTE " _
                & " FROM TMP_SOCIOS " _
                & " WHERE usu = '" + wcodusu + "' " _
                & " ORDER BY MESFALL, NOMBRE ")
   ...
   DataGrid1.Columns(5).Width = 1100   ' MESFALL
   DataGrid1.Columns(5).Alignment = dbgLeft
   DataGrid1.Columns(5).Caption = "MES FALL"
