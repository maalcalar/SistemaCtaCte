...
Begin VB.Form frmConSdosxConcepto 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Consulta de Saldos Por Concepto"
   ClientHeight    =   8355
   ClientLeft      =   45
   ClientTop       =   375
   ClientWidth     =   11565
   ...
   Begin VB.ComboBox cmbE_Socio 
      Height          =   315
      ItemData        =   "frmConSdosxConcepto.frx":0000
      Left            =   1320
      List            =   "frmConSdosxConcepto.frx":0002
   ...
   Begin VB.ComboBox cmbConcepto 
      Height          =   315
      ItemData        =   "frmConSdosxConcepto.frx":0004
      Left            =   1320
      List            =   "frmConSdosxConcepto.frx":0006
      Style           =   2  'Dropdown List
      TabIndex        =   2
      Top             =   480
      Width           =   3375
   ...
   Begin VB.ComboBox cmbCia 
      Height          =   315
      ItemData        =   "frmConSdosxConcepto.frx":0008
      Left            =   1320
      List            =   "frmConSdosxConcepto.frx":000A
      Style           =   2  'Dropdown List
      TabIndex        =   0
      Top             =   165
      Width           =   7335
   ...
   End
End
Attribute VB_Name = "frmConSdosxConcepto"
...
Private Sub cmdExportar_Click()
   ...
   Dim wNom As String, wSoc As Long, wMes As String, wcon As String, _
   ...
   With objExcel
        ...
        .Cells(1, 1) = wnomcia
        .Cells(2, 1) = "REPORTE DE SALDOS POR CONCEPTO - MES " + txtMes.Text
        ...
        objExcel.Columns("F").ColumnWidth = 35
   ...
   aa = Leerado3("SELECT * FROM TMP_CTASXCAB WHERE USU = '" + wcodusu + "' ORDER BY CONCEPTO, NOMBRE, MES ")
   If aa > 0 Then
      V = 4
      H = 1
      wRegAct = 1
      ...
      Do While Not ADO3.EOF
         ...
         Do While ADO3!concepto = wcon
            ...
            objExcel.Cells(V, H + 5) = ADO3!nomcon
            objExcel.Cells(V, H + 6) = ADO3!mes
         ...
         objExcel.Cells(V, H + 5) = "TOTALES X CONCEPTO"
         objExcel.Cells(V, H + 8) = wCargos
         objExcel.Cells(V, H + 9) = wAbonos
         objExcel.Cells(V, H + 10) = wSdoNew
         V = V + 2
      Loop
      If wCargos <> zCargos Or wAbonos <> zAbonos Or wSdoNew <> zSdoNew Then
         V = V + 1
         objExcel.Range(objExcel.Cells(V, H + 8), objExcel.Cells(V, H + 10)).NumberFormat = "####,##0.00;;\ "
         objExcel.Range(objExcel.Cells(V, H + 8), objExcel.Cells(V, H + 10)).Font.Bold = True
         objExcel.Range(objExcel.Cells(V, H + 8), objExcel.Cells(V, H + 10)).Font.Color = RGB(255, 0, 0)
         ...
         objExcel.Cells(V, H + 8) = zCargos
         objExcel.Cells(V, H + 9) = zAbonos
         objExcel.Cells(V, H + 10) = zSdoNew
      End If
      V = V + 1
...
Private Sub cmdImprimir_Click()
   ...
   Crys1.ReportFileName = xraiz + "ReportCtaCte\SdosxCon.RPT"
   Crys1.Formulas(0) = "NOMBRECIA= '" + wnomcia + "' "
   Crys1.Formulas(1) = "RUCCIA= 'RUC " + wruccia + "' "
   Crys1.Formulas(2) = "MESCORTE= 'AL MES DE " + wMes + "' "
...
Private Sub Form_Load()
   frmConSdosxConcepto.Left = (Screen.Width - Width) \ 2
   frmConSdosxConcepto.Top = 0
End Sub

Private Sub LlenaCab()
   Dim aa As Long, _
       wcon As String, WE_S As String, wMes As String, _
       wSoc As Integer, sw As String, wFec As Date, _
...
'   End If
   ...
   & " (CODSOCIO, MES, CONCEPTO, NOMBRE, NOMCON, E_SOCIO, MONEDA, " _
   & "  CARGOS, ABONOS, SDONEW, USU ) " _
   & " SELECT " _
   & "  C.CODSOCIO, C.MES, C.CONCEPTO, S.NOMBRE, M.NOMBRE, S.E_SOCIO, " _
   ...
   aa = Leerado2("SELECT NOMBRE, MES, NOMCON, E_SOCIO, " _
            & "          MONEDA, CARGOS, ABONOS, SDONEW, USU, " _
            & "          CONCEPTO, CODSOCIO " _
            & " FROM TMP_CTASXCAB " _
            & " WHERE USU = '" + wcodusu + "' " _
            & " ORDER BY CONCEPTO, NOMBRE, MES ")
   Set DataGrid1.DataSource = ADO2
   
   DataGrid1.Columns(0).Width = 3850  ' NOMBRE
   DataGrid1.Columns(0).Alignment = dbgLeft
   DataGrid1.Columns(0).Caption = "NOMBRE"
    
   DataGrid1.Columns(1).Width = 750   ' MESCOB
   DataGrid1.Columns(1).Alignment = dbgLeft
   DataGrid1.Columns(1).Caption = "MES"
    
   DataGrid1.Columns(2).Width = 2500  ' NOMCON
   DataGrid1.Columns(2).Alignment = dbgLeft
   DataGrid1.Columns(2).Caption = "CONCEPTO"
    
   DataGrid1.Columns(3).Width = 500   ' E_SOCIO
   DataGrid1.Columns(3).Alignment = dbgLeft
   DataGrid1.Columns(3).Caption = "E_SOCIO"
    
   DataGrid1.Columns(4).Width = 350   ' MONEDA
   DataGrid1.Columns(4).Alignment = dbgCenter
   DataGrid1.Columns(4).Caption = "MON"
    
   DataGrid1.Columns(5).Width = 850   ' CARGOS
   DataGrid1.Columns(5).Alignment = dbgRight
   DataGrid1.Columns(5).Caption = "CARGOS"
   DataGrid1.Columns(5).NumberFormat = "###,##0.00;;\ "
    
   DataGrid1.Columns(6).Width = 850   ' CARGOS
   DataGrid1.Columns(6).Alignment = dbgRight
   DataGrid1.Columns(6).Caption = "ABONOS"
   DataGrid1.Columns(6).NumberFormat = "###,##0.00;;\ "
    
   DataGrid1.Columns(7).Width = 850   ' SDONEW
