...
Begin VB.Form frmMaeGradoGrupo 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Maestro de Grupos de Grados"
   ClientHeight    =   8460
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10950
   ...
   Begin MSDataGridLib.DataGrid DataGrid1 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "TABLA DE GRUPOS DE GRADOS"
      ColumnCount     =   2
   ...
   End
End
Attribute VB_Name = "frmMaeGradoGrupo"
...
Sub refrescar()
   ...
   txtCodigo.Text = ADO1!gradogrupo
...
Sub grabar()
   ...
   aa = Leerado8("SELECT * FROM MAEGRADOGRUPO WHERE GRADOGRUPO = " + Str(Val(wCod)) + " ")
   If aa = 0 Then
      Db.BeginTrans
      Db.Execute ("INSERT INTO MAEGRADOGRUPO " _
      & " (GRADOGRUPO, NOMBRE) " _
      & " VALUES " _
      & " (" + Str(Val(wCod)) + ", '" + wNom + "' ) ")
      Db.CommitTrans
   Else
      Db.BeginTrans
      Db.Execute ("UPDATE MAEGRADOGRUPO " _
      & " SET NOMBRE = '" + wNom + "' " _
      & " WHERE GRADOGRUPO = " + Str(Val(wCod)) + " ")
      Db.CommitTrans
   End If
   ADO1.Requery
   LlenaCab1
   ADO1.Find "GRADOGRUPO = " + Str(Val(wCod)) + " "
   MsgBox "Grado Grupo " + wCod + " " + wNom + vbNewLine + _
...
Private Sub cmdEliminar_Click()
   ...
   aa = Leerado8("SELECT * FROM MAEGRADO WHERE GRADOGRUPO = " + Str(Val(wcon)) + " ")
   If aa > 0 Then
      MsgBox "Grado Grupo Tiene Movimiento, No Se Pude Borrar", vbExclamation
      Exit Sub
   End If
   
   If MsgBox("¿Esta seguro de borrar Codigo " + wcon + "?", vbYesNo + vbDefaultButton2 + vbQuestion, "Advertencia") = vbYes Then
      Db.BeginTrans
      Db.Execute ("DELETE FROM MAEGRADOGRUPO WHERE GRADOGRUPO = " + Str(wcon) + " ")
      Db.CommitTrans
      ...
      If wNew <> 0 Then
         ADO1.Find "GRADOGRUPO=" + Str(Val(wNew)) + ""
      End If
      MsgBox "GradoGrupo " + Str(wcon) + " " + wNom + vbNewLine + _
...
Private Sub cmdExporta_Click()
   ...
   Heading(0) = "CODIGO"
   Heading(1) = "NOMBRE"
   aa = Leerado3("SELECT * FROM MAEGRADOGRUPO ORDER BY GRADOGRUPO ")
   If aa > 0 Then
      wTot = aa
      ...
      With objExcel
           ...
           .Cells(1, 1) = wnomcia
           .Cells(2, 1) = "MAESTRO DE GRADOS GRUPOS"
           For I = 1 To 2 Step 1
      ...
      Do While Not ADO3.EOF
         ...
         lblMensaje.Refresh
         objExcel.Cells(V, H + 0) = ADO3!gradogrupo
...
Private Sub cmdGrabar_Click()
   ...
   If ACCION = 1 Then
      wCod = txtCodigo.Text
      If Leerado2("SELECT * FROM MAEGRADOGRUPO WHERE GRADOGRUPO = " + Str(Val(wCod)) + " ") > 0 Then
...
Private Sub Form_Activate()
   frmMaeGradoGrupo.Left = (Screen.Width - Width) \ 2
   frmMaeGradoGrupo.Top = 0
   optTodos.Value = True
   Dim a As Integer
...
'   optTodos.Value = True
   ...
   a = Leerado("SELECT GRADOGRUPO, NOMBRE " _
                & " FROM MAEGRADOGRUPO " _
                & " ORDER BY GRADOGRUPO ")
...
Private Sub txtCodigo_KeyPress(KeyAscii As Integer)
   ...
   If KeyAscii = 13 Then
      If txtCodigo = "" Then
         ...
         Exit Sub
      End If
      aa = Leerado8("SELECT * FROM MAEGRADOGRUPO WHERE GRADOGRUPO = " + Str(Val(txtCodigo.Text)) + " ")
      If aa > 0 Then
