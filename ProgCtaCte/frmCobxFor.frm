...
Begin VB.Form frmCobxFor 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Reporte de Cobranzas Por Forma de Pago"
   ClientHeight    =   8055
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   13845
   ...
   Begin VB.ComboBox cmbCia 
      Height          =   315
      ItemData        =   "frmCobxFor.frx":0000
      Left            =   1080
      List            =   "frmCobxFor.frx":0002
   ...
   Begin MSDataGridLib.DataGrid DataGrid1 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "RELACION DE COBROS POR FORMA DE PAGO"
      ColumnCount     =   2
...
Private Sub cmdImprimir_Click()
   ...
   Crys1.ReportFileName = xraiz + "ReportCtaCte\CobxFor.RPT"
...
Private Sub Form_Load()
   frmCobxFor.Left = (Screen.Width - Width) \ 2
   frmCobxFor.Top = 0
End Sub
...
Private Sub LlenaCab()
   ...
   & " ( TIPCOB, SERCOB, NUMCOB, LINCOB, FECHA, CODSOCIO, NOMBRE, DOLARE, SOLESS, " _
   & "   MESCOB, CONCEPTO, CONPAGO, NOMCON, MONDOC, FORPAG, " _
   & "   SDOOLD, ABONOS, SDONEW, IMPORTE, USU ) " _
   & " SELECT " _
   & "   D.TIPCOB, D.SERCOB, D.NUMCOB, D.LINCOB, C.FECHA, C.CODSOCIO, '', " _
   & "   D.DOLARE, D.SOLESS, D.MESCOB, D.CONCEPTO, D.CONPAGO, '', MONDOC, FORPAG, " _
   & "   D.SDOOLD, D.ABONOS, D.SDONEW, D.IMPORTE, '" + wcodusu + "' " _
   ...
   If wfor <> "" Then
      Db.BeginTrans
      Db.Execute ("DELETE FROM TMP_COBDET " _
   ...
   aa = Leerado2("SELECT FORPAG, CODSOCIO, NOMBRE, MESCOB, TIPCOB, SERCOB, NUMCOB, LINCOB, FECHA, " _
                & "      CONPAGO, DOLARE, SOLESS, MONDOC, " _
                & "      SDOOLD, ABONOS, SDONEW " _
                & " FROM TMP_COBXFEC " _
                & " WHERE USU = '" + wcodusu + "' " _
                & " ORDER BY FORPAG, TIPCOB, SERCOB, NUMCOB, LINCOB ")
   Set DataGrid1.DataSource = ADO2
End Sub

Private Sub LlenaCab1()
   DataGrid1.Columns(0).Width = 450   ' FORPAG
   DataGrid1.Columns(0).Alignment = dbgRight
   DataGrid1.Columns(0).Caption = "F.PG"
       
   DataGrid1.Columns(1).Width = 650   ' CODSOCIO
   DataGrid1.Columns(1).Alignment = dbgRight
   DataGrid1.Columns(1).Caption = "SOCIO"
       
   DataGrid1.Columns(2).Width = 3000  ' NOMBRE
   DataGrid1.Columns(2).Alignment = dbgLeft
   DataGrid1.Columns(2).Caption = "NOMBRE"
    
   DataGrid1.Columns(3).Width = 750   ' MESCOB
   DataGrid1.Columns(3).Alignment = dbgLeft
   DataGrid1.Columns(3).Caption = "MES"
    
   DataGrid1.Columns(4).Width = 350   ' TIPCOB
   DataGrid1.Columns(4).Alignment = dbgCenter
   DataGrid1.Columns(4).Caption = "TC"
    
   DataGrid1.Columns(5).Width = 480   ' SERCOB
   DataGrid1.Columns(5).Alignment = dbgCenter
   DataGrid1.Columns(5).Caption = "SERIE"
    
   DataGrid1.Columns(6).Width = 1020  ' NUMCOB
   DataGrid1.Columns(6).Alignment = dbgCenter
   DataGrid1.Columns(6).Caption = "NUM.COB."
   
   DataGrid1.Columns(7).Width = 350   ' LINCOB
   DataGrid1.Columns(7).Alignment = dbgCenter
   DataGrid1.Columns(7).Caption = "LIN"
   
   DataGrid1.Columns(8).Width = 1030   ' FECHA
   DataGrid1.Columns(8).Alignment = dbgCenter
   DataGrid1.Columns(8).NumberFormat = "dd/mm/yyyy"
   DataGrid1.Columns(8).Caption = "FECHA"
    
   DataGrid1.Columns(9).Width = 550   ' CONCEPTO
   DataGrid1.Columns(9).Alignment = dbgLeft
   DataGrid1.Columns(9).Caption = "CONC"
    
   DataGrid1.Columns(10).Width = 950     ' DOLARE'
   DataGrid1.Columns(10).Alignment = dbgRight
   DataGrid1.Columns(10).Caption = "  US$  "
   DataGrid1.Columns(10).NumberFormat = "####0.00"
   
   DataGrid1.Columns(11).Width = 950     ' SOLESS'
   DataGrid1.Columns(11).Alignment = dbgRight
   DataGrid1.Columns(11).Caption = "  S/.  "
   DataGrid1.Columns(11).NumberFormat = "####0.00"
   
   DataGrid1.Columns(12).Width = 380   ' MONDOC '
   DataGrid1.Columns(12).Alignment = dbgLeft
   DataGrid1.Columns(12).Caption = "MON"
       
   DataGrid1.Columns(13).Width = 850     ' SDOOLD'
   DataGrid1.Columns(13).Alignment = dbgRight
   DataGrid1.Columns(13).Caption = "SDO.OLD"
   DataGrid1.Columns(13).NumberFormat = "####0.00"

   DataGrid1.Columns(14).Width = 850     ' ABONOS'
   DataGrid1.Columns(14).Alignment = dbgRight
   DataGrid1.Columns(14).Caption = "COBROS"
   DataGrid1.Columns(14).NumberFormat = "####0.00"
       
   DataGrid1.Columns(15).Width = 850     ' SDONEW'
   DataGrid1.Columns(15).Alignment = dbgRight
   DataGrid1.Columns(15).Caption = "NUEVO SDO"
...
Private Sub txtForPag_Change()
   Dim aa As Integer
   aa = Leerado8("SELECT * FROM MAEFORPAG WHERE FORPAG = '" + txtForPag.Text + "' ")
   If aa > 0 Then
      lblForPag.Caption = ADO8!nombre
   Else
      lblForPag.Caption = ""
   End If
   Set ADO8 = Nothing
...
Private Sub txtForPag_KeyPress(KeyAscii As Integer)
   ...
   If KeyAscii = 13 Then
      If Len(Trim(txtForPag.Text)) > 0 Then
         aa = Leerado8("SELECT * FROM MAEFORPAG WHERE FORPAG = '" + txtForPag.Text + "' ")
         If aa = 0 Then
            MsgBox "Forma de Pago Digitado Es Invalido", vbExclamation
            txtForPag.Text = ""
            Exit Sub
         End If
      End If
