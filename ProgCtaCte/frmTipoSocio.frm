...
Begin VB.Form frmMaeTipoSocio 
   ...
   Begin VB.Frame Frame3 
      ...
      Top             =   0
      Width           =   8295
      Begin VB.CheckBox chkElec 
         Alignment       =   1  'Right Justify
         Caption         =   "Vota en Elecciones??"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
      ...
      Begin VB.Frame Frame4 
         Caption         =   "Datos de Aportación Mensual"
         ForeColor       =   &H000000FF&
         Height          =   975
         Left            =   3120
         TabIndex        =   25
   ...
   Begin MSDataGridLib.DataGrid DataGrid1 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "TABLA DE TIPOS DE SOCIOS"
      ColumnCount     =   2
   ...
   End
End
Attribute VB_Name = "frmMaeTipoSocio"
...
Sub refrescar()
   ...
   txtCodigo.Text = ADO1!e_socio
   txtNombre.Text = IIf(IsNull(ADO1!nombre), "", ADO1!nombre)
   chkElec.Value = IIf(ADO1!eleccion = True, 1, 0)
   txtMoneda.Text = IIf(IsNull(ADO1!moneda), "", ADO1!moneda)
   If IsNull(ADO1!aporte) Then
      txtAporte.Text = ""
   Else
      txtAporte.Text = Format(ADO1!aporte, "#####0.00")
   End If
End Sub
...
Sub grabar()
   ...
   Dim aa As Integer, _
       wCod As String, wNom As String, _
       wMon As String, wApo As Currency
   wCod = txtCodigo.Text
   wNom = txtNombre.Text
   wMon = txtMoneda.Text
   wApo = Val(txtAporte.Text)
   ...
   aa = Leerado8("SELECT * FROM MAEE_SOCIO WHERE E_SOCIO = '" + wCod + "' ")
   If aa = 0 Then
      Db.BeginTrans
      Db.Execute ("INSERT INTO MAEE_SOCIO " _
      & " (E_SOCIO, NOMBRE, ELECCION, MONEDA, APORTE) " _
      & " VALUES " _
      & " ('" + wCod + "', '" + wNom + "', " _
      & "  " + Str(IIf(chkElec.Value = vbChecked, 1, 0)) + ", " _
      & "  '" + wMon + "', " + Str(wApo) + " ) ")
      Db.CommitTrans
   Else
      Db.BeginTrans
      Db.Execute ("UPDATE MAEE_SOCIO " _
      & " SET NOMBRE = '" + wNom + "', " _
      & "     ELECCION = " + Str(IIf(chkElec.Value = vbChecked, 1, 0)) + ", " _
      & "     MONEDA = '" + wMon + "', APORTE = " + Str(wApo) + " " _
      & " WHERE E_SOCIO = '" + wCod + "' ")
      Db.CommitTrans
   End If
   ADO1.Requery
   LlenaCab1
   ADO1.Find "E_SOCIO = '" + wCod + "' "
   MsgBox "Tipo Socio '" + wCod + " " + wNom + "'" + vbNewLine + _
...
Private Sub editar(estado As Boolean)
   ...
   chkElec.Enabled = estado
   txtMoneda.Enabled = estado
   txtAporte.Enabled = estado
...
Private Sub cmdEliminar_Click()
   ...
   If MsgBox("¿Esta seguro de borrar Codigo '" + wcon + "'?", vbYesNo + vbDefaultButton2 + vbQuestion, "Advertencia") = vbYes Then
      Db.BeginTrans
      Db.Execute ("DELETE FROM MAEE_SOCIO WHERE E_SOCIO = '" + wcon + "' ")
      Db.CommitTrans
      ...
      If Len(Trim(wNew)) <> 0 Then
         ADO1.Find "E_SOCIO='" + wNew + "' "
      End If
      MsgBox "Tipo Socio '" + wcon + " " + wNom + "'" + vbNewLine + _
...
Private Sub cmdExporta_Click()
   ...
   Dim aa As Integer, I As Integer, Heading(4) As String, wreg As Integer, wTot As Integer
   Dim wNom As String
   Heading(0) = "CODIGO"
   Heading(1) = "NOMBRE"
   Heading(2) = "ELECCIONES"
   Heading(3) = "MONEDA"
   Heading(4) = "APORTE"
   aa = Leerado3("SELECT * FROM MAEE_SOCIO ORDER BY E_SOCIO ")
   If aa > 0 Then
      wTot = aa
      ...
      With objExcel
           ...
           .Range(.Cells(3, 1), .Cells(3, 5)).Borders.LineStyle = xlContinuous
           .Range(.Cells(3, 1), .Cells(3, 5)).Font.Bold = True
           .Cells(1, 1) = wnomcia
           .Cells(2, 1) = "MAESTRO DE TIPOS DE SOCIOS"
           For I = 1 To 5 Step 1
           ...
           objExcel.Columns("E").ColumnWidth = 15
      End With
      V = 4
      H = 1
      wreg = 1
      Do While Not ADO3.EOF
         ...
         objExcel.Cells(V, H + 2) = ADO3!eleccion
         objExcel.Cells(V, H + 3) = ADO3!moneda
         objExcel.Cells(V, H + 4) = ADO3!aporte
...
Private Sub cmdGrabar_Click()
   ...
   If ACCION = 1 Then
      wCod = txtCodigo.Text
      If Leerado2("SELECT * FROM MAEE_SOCIO WHERE E_SOCIO = '" + wCod + "' ") > 0 Then
...
Private Sub Form_Activate()
   frmMaeTipoSocio.Left = (Screen.Width - Width) \ 2
   frmMaeTipoSocio.Top = 0
   optTodos.Value = True
   Dim a As Integer
...
'   optTodos.Value = True
   ...
   a = Leerado("SELECT E_SOCIO, NOMBRE, ELECCION, MONEDA, APORTE " _
                & " FROM MAEE_SOCIO " _
                & " ORDER BY E_SOCIO ")
...
Private Sub LlenaCab1()
   ...
   DataGrid1.Columns(2).Caption = "ELECCIONES"
   ...
   DataGrid1.Columns(4).Caption = "APORTE"
End Sub
...
Private Sub txtAporte_GotFocus()
   txtAporte.SelStart = 0
   txtAporte.SelLength = 10
End Sub

Private Sub txtAporte_KeyDown(KeyCode As Integer, Shift As Integer)
   Select Case KeyCode
   Case 38
        txtMoneda.SetFocus
   End Select
...
Private Sub txtAporte_KeyPress(KeyAscii As Integer)
   If KeyAscii = 13 Then
      If Len(Trim(txtAporte.Text)) <> 0 Then
         txtAporte.Text = Format(txtAporte.Text, "####0.00;;\ ")
      End If
      cmdGrabar.SetFocus
   Else
...
Private Sub txtCodigo_KeyPress(KeyAscii As Integer)
   ...
   If KeyAscii = 13 Then
      If txtCodigo = "" Then
         ...
         Exit Sub
      End If
      aa = Leerado8("SELECT * FROM MAEE_SOCIO WHERE E_SOCIO = '" + txtCodigo.Text + "' ")
      If aa > 0 Then
         MsgBox "Tipo de Socio Ya Existe", vbExclamation
         txtCodigo.Text = ""
         Exit Sub
      End If
      txtNombre.SetFocus
...
Private Sub txtMoneda_KeyPress(KeyAscii As Integer)
   If KeyAscii = 13 Then
      If Len(Trim(txtMoneda.Text)) = 0 Then
         MsgBox "Tipo de Moneda En Blanco", vbInformation
         txtMoneda.Text = "S"
         Exit Sub
      End If
      txtAporte.SetFocus
   Else
      KeyAscii = Asc(UCase(Chr(KeyAscii)))
      If InStr(1, "SD" + Chr(8), Chr(KeyAscii)) = 0 Then
         KeyAscii = 0
      End If
   End If
End Sub
