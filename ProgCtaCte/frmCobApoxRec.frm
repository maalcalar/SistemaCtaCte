...
Begin VB.Form frmCobApoxRec 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Reporte de Cobranzas Aportaciones Por Número de Recibo"
   ClientHeight    =   8055
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   16605
   ...
   End
End
Attribute VB_Name = "frmCobApoxRec"
...
Private Sub cmdExportar_Click()
   ...
   Dim aa As Integer, I As Integer, Heading(9) As String, _
   ...
   If aa > 0 Then
      ...
      Do While Not ADO3.EOF
         ...
         objExcel.Range(objExcel.Cells(V, H + 8), objExcel.Cells(V, H + 9)).NumberFormat = "######0.00;;\ "
         ...
         objExcel.Cells(V, H + 7) = ADO3!glosa
         objExcel.Cells(V, H + 8) = ADO3!dolare
         objExcel.Cells(V, H + 9) = ADO3!soless
      ...
      objExcel.Cells(V, H + 8) = wdol
      objExcel.Cells(V, H + 9) = wsol
      V = V + 1
...
Private Sub Form_Load()
   frmCobApoxRec.Left = (Screen.Width - Width) \ 2
   frmCobApoxRec.Top = 0
End Sub
...
Private Sub LlenaCab()
   ...
   Db.BeginTrans
   Db.Execute ("DELETE FROM TMP_COBXREC WHERE USU = '" + wcodusu + "' ")
   Db.CommitTrans

   Db.BeginTrans
   Db.Execute ("INSERT INTO TMP_COBXREC " _
   & " ( FECHA, TIPCOB, SERCOB, NUMCOB, LINCOB, CODIGO, INS, NOMBRE, E_SOCIO, CONPAGO, " _
   & "   NOMPAGO, DOLARE, SOLESS, GLOSA, USU ) " _
   & " SELECT " _
   & "  C.FECHA, D.TIPCOB, D.SERCOB, D.NUMCOB, D.LINCOB, M.CODIGO, M.INS, M.NOMBRE, M.E_SOCIO, " _
   & "  D.CONPAGO, O.DESCONCE, SUM(D.DOLARE), SUM(D.SOLESS), C.GLOSA, '" + wcodusu + "' " _
   & " FROM COBRODET AS D INNER JOIN COBROCAB     AS C ON D.ANO = C.ANO AND D.TIPCOB = C.TIPCOB AND D.SERCOB = C.SERCOB AND D.NUMCOB = C.NUMCOB " _
   & "                    INNER JOIN MAESOCIO     AS M ON C.CODSOCIO = M.CODSOCIO " _
   & "                    INNER JOIN ZZZ_concepto AS O ON D.CONPAGO = O.CONCEPTO " _
   & " WHERE O.APORTE = 1 AND " _
   & "       FECHA >= '" + Format(wdes, "dd/mm/yyyy") + "' AND " _
   & "       FECHA <= '" + Format(whas, "dd/mm/yyyy") + "' " _
   & " GROUP BY D.TIPCOB, D.SERCOB, D.NUMCOB, D.LINCOB, C.FECHA, M.CODIGO, M.INS, M.NOMBRE, M.E_SOCIO, D.CONPAGO, O.DESCONCE, C.GLOSA ")
   Db.CommitTrans
...
Private Sub LlenaCab1()
   ...
   DataGrid1.Columns(8).Width = 4900   ' glosa
