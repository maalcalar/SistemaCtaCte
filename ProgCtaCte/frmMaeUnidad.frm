...
Begin VB.Form frmMaeUnidad 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Maestro de Unidades Policiales"
   ClientHeight    =   8460
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10950
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.ComboBox cmbRegion 
         Height          =   315
         ItemData        =   "frmMaeUnidad.frx":0000
         Left            =   1800
         List            =   "frmMaeUnidad.frx":0002
   ...
   End
End
Attribute VB_Name = "frmMaeUnidad"
...
Sub refrescar()
   ...
   txtCodigo.Text = ADO1!unidad
   txtNombre.Text = IIf(IsNull(ADO1!nombre), "", ADO1!nombre)
   txtAbrev.Text = IIf(IsNull(ADO1!abrev), "", ADO1!abrev)
   cmbRegion.ListIndex = 0
End Sub

Private Function BuscaRegion(zReg As String) As Integer
   On Error GoTo err
   
   Dim yy As Integer, RegAct As Integer, RegTot As Integer, marca As Integer

   marca = 0
   yy = Leerado5a("SELECT * FROM MAEREGION ORDER BY REGION")
   If yy > 0 Then
      ADO5a.MoveFirst
      RegAct = 1
      RegTot = yy
      Do While Not ADO5a.EOF
         If ADO5a!region = zReg Then
            marca = RegAct
            Exit Do
         End If
...
Sub grabar()
   ...
   Dim aa As Integer, _
       wCod As String, wNom As String, wAbr As String, wreg As String
   wCod = txtCodigo.Text
   wNom = txtNombre.Text
   wAbr = txtAbrev.Text
   wreg = cmbRegion.ItemData(cmbRegion.ListIndex)
   ...
   If Len(Trim(wAbr)) = 0 Then
      MsgBox "Nombre Abreviado En Blanco", vbExclamation
      Exit Sub
   End If
   
   If Len(Trim(wreg)) = 0 Then
      MsgBox "region En Blanco", vbExclamation
      Exit Sub
   End If
   
   aa = Leerado8("SELECT * FROM MAEUNIDAD WHERE CODIGO = " + Str(Val(wCod)) + " ")
   If aa = 0 Then
      Db.BeginTrans
      Db.Execute ("INSERT INTO MAEUNIDAD " _
      & " (UNIDAD, NOMBRE, ABREV, REGION) " _
      & " VALUES " _
      & " (" + Str(Val(wCod)) + ", '" + wNom + "', '" + wAbr + "', '" + wreg + "'  ) ")
      Db.CommitTrans
   Else
      Db.BeginTrans
      Db.Execute ("UPDATE MAEUNIDAD " _
      & " SET NOMBRE = '" + wNom + "', ABREV = '" + wAbr + "', REGION = '" + wreg + "'  " _
      & " WHERE UNIDAD = " + Str(Val(wCod)) + " ")
      Db.CommitTrans
   End If
   ADO1.Requery
   LlenaCab1
   ADO1.Find "UNIDAD = " + Str(Val(wCod)) + " "
   MsgBox "Unidad de Pago " + wCod + " " + wNom + vbNewLine + _
...
Private Sub cmdEliminar_Click()
   ...
   aa = Leerado8("SELECT * FROM MAESOCIO WHERE UNIDAD = " + Str(Val(wcon)) + " ")
   If aa > 0 Then
      MsgBox "Unidad de Pago Tiene Movimiento, No Se Pude Borrar", vbExclamation
      Exit Sub
   End If
   
   If MsgBox("¿Esta seguro de borrar Codigo " + wcon + "?", vbYesNo + vbDefaultButton2 + vbQuestion, "Advertencia") = vbYes Then
      Db.BeginTrans
      Db.Execute ("DELETE FROM MAEUNIDAD WHERE UNIDAD = " + Str(wcon) + " ")
      Db.CommitTrans
      ...
      If wNew <> 0 Then
         ADO1.Find "UNIDAD=" + Str(Val(wNew)) + ""
      End If
      MsgBox "Unidad " + Str(wcon) + " " + wNom + vbNewLine + _
...
Private Sub cmdExporta_Click()
   ...
   Dim aa As Integer, I As Integer, Heading(1) As String, wreg As Integer, wTot As Integer
   Dim wNom As String
   Heading(0) = "CODIGO"
   Heading(1) = "NOMBRE"
   aa = Leerado3("SELECT * FROM MAEGRADO ORDER BY CODIGO ")
   If aa > 0 Then
      wTot = aa
...
Private Sub cmdGrabar_Click()
   ...
   If ACCION = 1 Then
      wCod = txtCodigo.Text
      If Leerado2("SELECT * FROM MAEUNIDAD WHERE UNIDAD = " + Str(Val(wCod)) + " ") > 0 Then
...
Private Sub Form_Activate()
   frmMaeUnidad.Left = (Screen.Width - Width) \ 2
   frmMaeUnidad.Top = 0
   optTodos.Value = True
   Dim a As Integer, I As Integer
   
   a = Leerado8("SELECT * FROM MAEREGION ORDER BY REGION ")
   If a > 0 Then
      ReDim aRegiones(a)
      ...
      Do While Not ADO8.EOF
         ...
         cmbRegion.AddItem ADO8!region + " " + ADO8!nombre
         cmbRegion.ItemData(I) = ADO8!region
...
'   optTodos.Value = True
   ...
   a = Leerado("SELECT UNIDAD, NOMBRE, ABREV, REGION " _
                & " FROM MAEUNIDAD " _
                & " ORDER BY UNIDAD ")
...
Private Sub txtAbrev_KeyDown(KeyCode As Integer, Shift As Integer)
   Select Case KeyCode
   Case 38
        txtNombre.SetFocus
   End Select
...
Private Sub txtCodigo_KeyPress(KeyAscii As Integer)
   ...
   If KeyAscii = 13 Then
      If txtCodigo = "" Then
         ...
         Exit Sub
      End If
      aa = Leerado8("SELECT * FROM MAEUNIDAD WHERE UNIDAD = " + Str(Val(txtCodigo.Text)) + " ")
      If aa > 0 Then
         MsgBox "Codigo de Unidad Ya Existe", vbExclamation
         txtCodigo.Text = ""
         Exit Sub
      End If
      txtNombre.SetFocus
