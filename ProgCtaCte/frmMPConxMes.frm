...
Begin VB.Form frmMPConxMes 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Consulta Caja Militar Policial Por Mes"
   ClientHeight    =   7980
   ClientLeft      =   45
   ClientTop       =   375
   ClientWidth     =   13350
   ...
   Begin VB.ComboBox cmbMeses 
      Height          =   315
      ItemData        =   "frmMPConxMes.frx":0000
      Left            =   720
      List            =   "frmMPConxMes.frx":0002
   ...
   End
End
Attribute VB_Name = "frmMPConxMes"
...
Private Sub cmdBuscar_Click()
   ...
   & "   TOTASIG1, TOTASIG2, TOTASIG3, TOTASIG4, TOTASIG5, " _
   & "   CODASIG1, CODASIG2, CODASIG3, CODASIG4, CODASIG5, " _
   & "   '', '', '', '', '', TOTENVIO, DSCCAJMP, DSCDIFER, TIPCOB  , '" + wcodusu + "'  " _
   & " FROM CAJMPCAB " _
   & " WHERE MES = '" + wAno + wMes + "'  ")
   Db.CommitTrans
   ...
   zz = Leerado2("SELECT CODSOCIO, NOMBRE  , TOTENVIO, DSCCAJMP, DSCDIFER, " _
                ...
                & "      TIPCOB  , USU     , MES     , CODIGO  , INS     , CARNETPNP, " _
                & "      NUMDOC  , CODBENI , FECENV  , FECDSC  " _
                & " FROM TMP_CAJMPCAB " _
...
Private Sub TotalCab()
   ...
   zTotEnvio = 0: zDscCajmp = 0: zDscDifer = 0
   zCanApo = 0: zCanAsi = 0
   ...
   If zz > 0 Then
      ...
      zDscCajmp = IIf(IsNull(ADO8!dsccajmp), 0, ADO8!dsccajmp)
   ...
   zz = Leerado8("SELECT COUNT(*) AS CAN " _
                ...
                & " WHERE (USU = '" + wcodusu + "') AND " _
                & "       (MES = '" + zAno + zMes + "') AND " _
                & "       (TOTASIG1 <> 0 OR TOTASIG2 <> 0 OR TOTASIG3 <> 0 OR " _
                & "        TOTASIG4 <> 0 OR TOTASIG5 <> 0 ) ")
   If zz > 0 Then
   ...
   lblDscCajMP.Caption = Format(zDscCajmp, "####,##0.00;;\ ")
...
Private Sub cmdExportar_Click()
   ...
   Heading(10) = "DEUDAS"
   Heading(11) = "ADELANTO"
   Heading(12) = "TOT.ENVIO"
   Heading(13) = "TOT.DSCTO"
   Heading(14) = "NO COBRADO"
   ...
   If aa > 0 Then
      ...
      Do While Not ADO3.EOF
         ...
         objExcel.Range(objExcel.Cells(V, H + 9), objExcel.Cells(V, H + 14)).NumberFormat = "#####,##0.00"
         ...
         objExcel.Cells(V, H + 8) = ADO3!e_socio
         objExcel.Cells(V, H + 9) = ADO3!totaport
         objExcel.Cells(V, H + 10) = ADO3!totdeuda
         objExcel.Cells(V, H + 11) = ADO3!totadela
         objExcel.Cells(V, H + 12) = ADO3!netsocio
         objExcel.Cells(V, H + 13) = ADO3!dscsocio
         objExcel.Cells(V, H + 14) = ADO3!difsocio
      ...
      objExcel.Range(objExcel.Cells(V, H + 9), objExcel.Cells(V, H + 14)).NumberFormat = "#####,##0.00"
      objExcel.Range(objExcel.Cells(V, H + 7), objExcel.Cells(V, H + 14)).Font.Color = RGB(255, 0, 0)
      objExcel.Range(objExcel.Cells(V, H + 7), objExcel.Cells(V, H + 14)).Font.Bold = True
      objExcel.Range(objExcel.Cells(V, H + 7), objExcel.Cells(V, H + 14)).Borders.Color = RGB(255, 0, 0)
            
      objExcel.Cells(V, H + 7) = "TOTALES FINALES"
      objExcel.Cells(V, H + 9) = wTotAport
      objExcel.Cells(V, H + 10) = wTotDeuda
      objExcel.Cells(V, H + 11) = wTotAdela
      objExcel.Cells(V, H + 12) = wNetSocio
      objExcel.Cells(V, H + 13) = wDscSocio
      objExcel.Cells(V, H + 14) = wDifSocio
...
Private Sub cmdImprimir_Click()
   ...
   Crys1.ReportFileName = xraiz + "ReportCtaCte\CajaMPxMes.RPT"
...
Private Sub Form_Initialize()
   frmMPConxMes.Left = (Screen.Width - Width) \ 2
   frmMPConxMes.Top = 0
...
Private Sub CalxSoc()
   ...
   & " (MES, CODENVIO, LIN, CODSOCIO, CODIGO, INS, CARNETPNP, NUMDOC, CODBENI, NOMBRE, E_SOCIO, " _
   ...
   & "  C.CODBENI, M.NOMBRE, M.E_SOCIO, C.FECENV, C.FECDSC, C.TOTAPORT, C.TOTDEUDA, " _
   ...
   & "  M.CODBENI, M.NOMBRE, M.E_SOCIO, C.FECENV, C.FECDSC, C.TOTASIG1, C.DEUASIG1, " _
   ...
   & "  M.CODBENI, M.NOMBRE, M.E_SOCIO, C.FECENV, C.FECDSC, C.TOTASIG2, C.DEUASIG2, " _
   ...
   & "  M.CODBENI, M.NOMBRE, M.E_SOCIO, C.FECENV, C.FECDSC, C.TOTASIG3, C.DEUASIG3, " _
   ...
   & "  M.CODBENI, M.NOMBRE, M.E_SOCIO, C.FECENV, C.FECDSC, C.TOTASIG4, C.DEUASIG4, " _
   ...
   & "  M.CODBENI, M.NOMBRE, M.E_SOCIO, C.FECENV, C.FECDSC, C.TOTASIG5, C.DEUASIG5, " _
