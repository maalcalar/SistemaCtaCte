...
Sub grabar()
   ...
   Dim aa As Integer, _
       ...
       wMesIni As String, wMesFecIng As String, wAnoFecIng As String
...
' Valida CODOFIN
   ...
   If aa = 0 Then
      ...
      & "  OBSERVAC, OBSERVA2, NRESO_REING, DIRECTIVO, ANODIREC, PROMOCION, USU ) " _
      & " VALUES " _
      & " (" + Str(Val(wSoc)) + ", " + Str(wCod) + ", " + Str(wIns) + ", '" + wNumDoc + "', " + Str(wCarPNP) + ", " _
      & "  '" + wCarPIP + "', '" + wNom + "', " + Str(wGrado) + ", " + Str(wSitua) + ", " + Str(wSituEsp) + ", " _
      & "  '" + wSexo + "', '" + wEcivi + "', '" + wDir + "', '" + wUbigeo + "', " _
      ...
      & "  '" + wDirectivo + "', '" + wAnoDirec + "', '" + wPromocion + "', '" + wcodusu + "' ) ")
      Db.CommitTrans
   Else
      Db.BeginTrans
...
Private Sub LlenaCab()
   ...
   & "  DIRECTIVO, ANODIREC, PROMOCION, NRESO_REING, USU ) " _
   & " SELECT " _
   & "  CODSOCIO, CODIGO, INS, NUMDOC, CARNETPNP, CARNETPIP, NOMBRE, GRADO, SITU, " _
   & "  SITUESP, FECNAC, FECMAT, SEXO, ECIVIL, DIREC, UBIGEO, TELEFONO, TELEFON2, " _
   & "  CELULAR, EMAIL, EMAIL2, REFER, E_SOCIO, TOMO, TIPCOB, FECING, FECRENU, " _
   & "  NRESO_ING, FRESO_ING, FECREIN, OBSERVAC, OBSERVA2, " _
   & "  ASIGNADO, FAMILIAR, VIP, FECVIP, CARTADIECO, FECAMNIS, FECCONDO, " _
   & "  DIRECTIVO, ANODIREC, PROMOCION, NRESO_REING, '" + wcodusu + "' " _
   & " FROM MAESOCIO ")
   Db.CommitTrans
   ...
   a = Leerado("SELECT CODSOCIO, CODIGO, INS, NUMDOC, NOMBRE, NOMGRA, NOMSITU, SEXO, ECIVIL, " _
                ...
                & "    ANODIREC, PROMOCION, NRESO_REING, USU  " _
                & " FROM TMP_MAESOCIO " _
                & " WHERE USU = '" + wcodusu + "' " _
                & " ORDER BY CODSOCIO ")
...
Private Sub txtIns_KeyPress(KeyAscii As Integer)
   ...
   If KeyAscii = 13 Then
      ...
      If aa > 0 Then
         ...
         MsgBox "Codofin Ya Existe En Socio" + Str(ADO8!codsocio) + vbNewLine + _
                "Codofin " + Str(ADO8!codigo) + "-" + Str(ADO8!ins) + vbNewLine + _
                wNom, vbExclamation
         txtCodofin.Text = ""
         txtIns.Text = ""
         txtCodofin.SetFocus
...
Private Sub txtNombre_KeyPress(KeyAscii As Integer)
   Dim aa As Integer, wNom As String, wSoc As Integer, wCod As Long, wIns As Integer
   If KeyAscii = 13 Then
      ...
      aa = Leerado8("SELECT * FROM MAESOCIO WHERE NOMBRE LIKE '" + wNom + "%' AND (CODSOCIO <> " + Str(wSoc) + " OR CODIGO <> " + Str(wCod) + " OR INS <> " + Str(wIns) + ")  ")
      If aa > 0 Then
         MsgBox "Ya Existe Nombre Digitado en Socio " + Str(ADO8!codsocio) + vbNewLine + _
                "Codofin " + Str(ADO8!codigo) + "-" + Str(ADO8!ins) + vbNewLine + _
                Trim(ADO8!nombre), vbExclamation
         txtNombre.Text = ""
         Exit Sub
      End If
