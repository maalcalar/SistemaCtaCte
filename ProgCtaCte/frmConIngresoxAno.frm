...
Begin VB.Form frmConIngresoxAno 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Relacion de Ingresos x Año"
   ClientHeight    =   8025
   ClientLeft      =   45
   ClientTop       =   390
   ClientWidth     =   12180
   ...
   End
End
Attribute VB_Name = "frmConIngresoxAno"
...
Private Sub cmdExportar_Click()
   ...
   Heading(6) = "FEC.RESOL"
   Heading(7) = "NUM.RESOL"
   aa = Leerado3("SELECT * FROM TMP_SOCIOS WHERE USU = '" + wcodusu + "' ORDER BY FECING, NOMBRE ")
   If aa > 0 Then
      wTot = aa
      ...
      With objExcel
           ...
           .Cells(1, 1) = wnomcia
           .Cells(2, 1) = "RELACION DE ASOCIADOS INGRESANTES - EJERCICIO " + txtAno.Text
           For I = 1 To 8 Step 1
      ...
      Do While Not ADO3.EOF
         ...
         End If
         If IsDate(ADO3!freso_ing) Then
            wFec = Format(ADO3!freso_ing, "dd/mm/yyyy")
            objExcel.Cells(V, H + 6) = wFec
         End If
         objExcel.Cells(V, H + 7) = ADO3!nreso_ing
         wreg = wreg + 1
         V = V + 1
         ADO3.MoveNext
      Loop
...
Private Sub cmdImprimir_Click()
   ...
   Crys1.ReportFileName = xraiz + "ReportCtaCte\RepIngxAno.RPT"
...
Private Sub Form_Initialize()
   frmConIngresoxAno.Left = (Screen.Width - Width) \ 2
   frmConIngresoxAno.Top = 0
End Sub
...
Private Sub LlenaCab()
   ...
   & " (CODSOCIO, CODIGO, INS, NOMBRE, E_SOCIO, FECING, MONEDA, APORTE, " _
   & "  FRESO_ING, NRESO_ING, USU ) " _
   & " SELECT " _
   & "  S.CODSOCIO, S.CODIGO, S.INS, S.NOMBRE, S.E_SOCIO, S.FECING, " _
   & "  E.MONEDA, E.APORTE, FRESO_ING, NRESO_ING, '" + wcodusu + "' " _
   & " FROM MAESOCIO AS S INNER JOIN MAEE_SOCIO AS E " _
   & "   ON S.E_SOCIO = E.E_SOCIO " _
   & " WHERE YEAR(S.FECING) =  " + Str(Val(wAno)) + "  ")
   Db.CommitTrans

   aa = Leerado2("SELECT CODSOCIO, CODIGO, INS, NOMBRE, E_SOCIO, FECING, " _
                & "      FRESO_ING, NRESO_ING, MONEDA, APORTE " _
                & " FROM TMP_SOCIOS " _
                & " WHERE usu = '" + wcodusu + "' " _
                & " ORDER BY FECING, NOMBRE ")
   ...
   DataGrid1.Columns(5).Caption = "FECING"
   DataGrid1.Columns(5).NumberFormat = "dd/mm/yyyy"
    
   DataGrid1.Columns(6).Width = 1100   ' FRESO_ING
   DataGrid1.Columns(6).Alignment = dbgLeft
   DataGrid1.Columns(6).Caption = "FEC.RESOL"
   DataGrid1.Columns(6).NumberFormat = "dd/mm/yyyy"
    
   DataGrid1.Columns(7).Width = 1100   ' NRESO_ING
   DataGrid1.Columns(7).Alignment = dbgRight
   DataGrid1.Columns(7).Caption = "NUM.RESOL"
