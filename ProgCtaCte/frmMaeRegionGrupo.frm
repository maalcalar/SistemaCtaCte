...
Begin VB.Form frmMaeRegionGrupo 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Maestro de Grupos de Regiones"
   ClientHeight    =   8460
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10950
   ...
   Begin MSDataGridLib.DataGrid DataGrid1 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "TABLA DE GRUPOS DE REGIONES"
      ColumnCount     =   2
   ...
   End
End
Attribute VB_Name = "frmMaeRegionGrupo"
...
Sub refrescar()
   ...
   txtCodigo.Text = ADO1!regiongrupo
...
Sub grabar()
   ...
   aa = Leerado8("SELECT * FROM MAEREGIONGRUPO WHERE REGIONGRUPO = " + Str(Val(wCod)) + " ")
   If aa = 0 Then
      Db.BeginTrans
      Db.Execute ("INSERT INTO MAEREGIONGRUPO " _
      & " (NOMBRE) " _
      & " VALUES " _
      & " ('" + wNom + "' ) ")
      Db.CommitTrans
   Else
      Db.BeginTrans
      Db.Execute ("UPDATE MAEREGIONGRUPO " _
      & " SET NOMBRE = '" + wNom + "' " _
      & " WHERE REGIONGRUPO = " + Str(Val(wCod)) + " ")
      Db.CommitTrans
   End If
   ADO1.Requery
   LlenaCab1
   ADO1.Find "REGIONGRUPO = " + Str(Val(wCod)) + " "
   MsgBox "Region Grupo " + wCod + " " + wNom + vbNewLine + _
...
Private Sub cmdEliminar_Click()
   ...
   wcon = ADO1!regiongrupo
   wNom = Trim(ADO1!nombre)
   wNew = 0
   ADO1.MoveNext
   If Not ADO1.EOF Then
      wNew = ADO1!regiongrupo
   Else
      ADO1.MovePrevious
      ADO1.MovePrevious
      If ADO1.BOF Then
   ...
   aa = Leerado8("SELECT * FROM MAEREGION WHERE REGIONGRUPO = " + Str(Val(wcon)) + " ")
   If aa > 0 Then
      MsgBox "Región Grupo Tiene Movimiento, No Se Pude Borrar", vbExclamation
      Exit Sub
   End If
   
   If MsgBox("¿Esta seguro de borrar Codigo " + Str(wcon) + "?", vbYesNo + vbDefaultButton2 + vbQuestion, "Advertencia") = vbYes Then
      Db.BeginTrans
      Db.Execute ("DELETE FROM MAEREGIONGRUPO WHERE REGIONGRUPO = " + Str(wcon) + " ")
      Db.CommitTrans
      ...
      If wNew <> 0 Then
         ADO1.Find "REGIONGRUPO=" + Str(Val(wNew)) + ""
      End If
      MsgBox "RegionGrupo " + Str(wcon) + " " + wNom + vbNewLine + _
...
Private Sub cmdExporta_Click()
   ...
   Heading(0) = "CODIGO"
   Heading(1) = "NOMBRE"
   aa = Leerado3("SELECT * FROM MAEREGIONGRUPO ORDER BY REGIONGRUPO ")
   If aa > 0 Then
      wTot = aa
      ...
      With objExcel
           ...
           .Cells(1, 1) = wnomcia
           .Cells(2, 1) = "MAESTRO DE REGION GRUPOS"
           For I = 1 To 2 Step 1
      ...
      Do While Not ADO3.EOF
         ...
         lblMensaje.Refresh
         objExcel.Cells(V, H + 0) = ADO3!regiongrupo
...
Private Sub cmdGrabar_Click()
   ...
   If ACCION = 1 Then
      wCod = txtCodigo.Text
      If Leerado2("SELECT * FROM MAEREGIONGRUPO WHERE REGIONGRUPO = " + Str(Val(wCod)) + " ") > 0 Then
...
Private Sub Form_Activate()
   frmMaeRegionGrupo.Left = (Screen.Width - Width) \ 2
   frmMaeRegionGrupo.Top = 0
   optTodos.Value = True
   Dim a As Integer
...
'   optTodos.Value = True
   ...
   a = Leerado("SELECT REGIONGRUPO, NOMBRE " _
                & " FROM MAEREGIONGRUPO " _
                & " ORDER BY REGIONGRUPO ")
...
Private Sub txtCodigo_KeyPress(KeyAscii As Integer)
   ...
   If KeyAscii = 13 Then
      If txtCodigo = "" Then
         ...
         Exit Sub
      End If
      aa = Leerado8("SELECT * FROM MAEREGIONGRUPO WHERE REGIONGRUPO = " + Str(Val(txtCodigo.Text)) + " ")
      If aa > 0 Then
